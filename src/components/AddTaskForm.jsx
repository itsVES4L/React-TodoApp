...
const AddTaskForm = ({ setIsAddNewTask, isAddNewTask }) => {
  ...
  const doneHandler = (e) => {
    ...
    if (addedSubTask.length >= 1) {
      ...
      setTitle("");
      setAddedSubTask([]);
      dispatch(addTask({ title: title || "newTask", subTasks: addedSubTask }));
    }
  ...
  const addSubTaskHandler = (e) => {
    ...
    if (currentSubTask.trim() !== "") {
      setAddedSubTask([
        ...addedSubTask,
        { subTask: currentSubTask.trim(), isCompleted: false, id: uuidv4() },
      ]);
      setCurrentSubTask("");
    }
  };
    ...
    <div className="w-full text-[#84849D]">
      <div className="flex flex-col gap-5 w-full mb-6 justify-center items-center">
        <p className="text-xs flex justify-center gap-2 items-center">
          <span className="font-bold text-[#8d8fd2]">note :</span> add all your tasks and then click the done button
        </p>
        ...
        <div className="w-[80%] flex flex-col gap-4 mt-3">
          <p>
            the <span className="text-[#8d8fd2] font-bold">tasks</span> you added :
          </p>
          <div className="w-[70%] flex overflow-y-scroll overflow-x-hidden pt-2 scrollbar-thumb-[#8d8fd2] scrollbar-thin md:h-[200px] justify-start pl-3 pb-2 flex-col gap-2">
            ...
            {addedSubTask.map((item, index) => (
              <p key={index}>
                <span className="text-[#8d8fd2] font-bold">{index + 1} . </span>
                {item.subTask}
              </p>
            ))}
          </div>
        </div>
        <div className="flex w-[50%] md:w-[70%] md:px-0 justify-center md:absolute md:right-12 md:bottom-10 md:justify-end gap-4">
          <button
            onClick={doneHandler}
